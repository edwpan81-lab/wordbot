...
**用户输入单词后**：
1. **查询检查**：在飞书表格中查找该单词是否存在
2. **判断分支**：
   - **已存在** → 显示现有信息，允许用户编辑/删除
   - **不存在** → 自动录入新单词
...
| 操作 | 说明 |
|------|------|
| 查看 | 显示现有中英文释义、例句、干扰词、状态 |
| 编辑 | 用户修改后更新到飞书表格（包含状态） |
| 删除 | 从飞书表格中删除该单词 |
...
| 仓库 | GitHub | 部署平台 | 说明 |
|------|--------|----------|------|
| 后端 | wordbot | Render Web Service | Start Command: `node server.js` |
| 前端 | wordbot-web | Render Static Site | Expo 构建产物 |

### 9.2 部署注意事项

**后端 (wordbot)**：
- 目录结构：`backend/`
- 启动命令：`node server.js`（不是 `node backend/server.js`）
- 必需环境变量：FEISHU_APP_ID, FEISHU_APP_SECRET, MINIMAX_API_KEY

**前端 (wordbot-web)**：
- Expo 构建后复制到仓库根目录
- API 地址硬编码为 Render 后端地址
- 静态托管，无需构建命令
...
| 问题 | 原因 | 解决方案 |
|------|------|----------|
| 部署后 start 命令找不到文件 | 命令设置为 `node backend/start` | 改为 `node server.js` |
| 路径重复 `backend/backend/` | 命令中包含了目录名 | 启动时已在 backend 目录 |
| 前端请求 localhost:3000 | API 地址指向本地 | 修改为生产环境域名 |
